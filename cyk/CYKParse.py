            ...
            if p!=-10:
                ...
                if getP(Y, 0, 0)!=0 and flag==0:
                    ...
                    if PYZ > getP(X, 0, 0):
                        flag = 1
                        P[X + '/' + str(0) + '/' + str(0)] = PYZ
                        T[X + '/' + str(0) + '/' + str(0)] = Tree.Tree(X, T[Y + '/' + str(0) + '/' + str(0)],None)
                PYZ = getP(Y, k, k)
                if PYZ > getP(X, k, k):
                    #print("重点存入了", X + '/' + str(k) + '/' + str(k))
                    P[X + '/' + str(k) + '/' + str(k)] = PYZ
                    T[X + '/' + str(k) + '/' + str(k)] = Tree.Tree(X, T[Y + '/' + str(k) + '/' + str(k)], None)
...
def getGrammarWeather():
    return {
        'syntax' : [
            ...
            ['NP+AdverbPhrase', 'Noun', 'AdverbPhrase', 0.8],
            ...
            ['AdverbPhrase', 'Preposition', 'NP', 0.1],
            ['AdverbPhrase', 'Preposition', 'Name', 0.2],
            ['AdverbPhrase', 'Adjective', 'AdverbPhrase', 0.2],
            ['AdverbPhrase', 'conjun', 'AdverbPhrase', 0.2],
...
if __name__ == '__main__':
    ...
    # CYKParse2(['hi', 'I', 'am', 'Peter'], getGrammarWeather())


    CYKParse(['what', 'is', 'the', 'temperature', 'in', 'Irvine', 'now'], getGrammarWeather())
   #CYKParse(['what', 'is', 'temperature', 'in', 'Irvine', 'now'], getGrammarWeather())
